...
"""Funcion para mostrar los .pkpass disponibles y escoger cual enviar"""
def menu_archivos(directorio_pkpass):
    archivos = os.listdir(directorio_pkpass)
    ...
    while not opcion.isdigit() or int(opcion) < 1 or int(opcion) > len(archivos):
        print("Selección inválida. Intente nuevamente.")
        opcion = input("Seleccione el número del archivo: ")

    # Obtener el archivo seleccionado
    archivo_seleccionado = archivos[int(opcion) - 1]
    file_to_send = os.path.join(directorio_pkpass, archivo_seleccionado)

    # Realizar las acciones deseadas con el archivo seleccionado
    print("\nArchivo seleccionado:", file_to_send)
    return file_to_send
...
file_to_send = menu_archivos(directorio_pkpass)
